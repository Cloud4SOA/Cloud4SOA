 ...
 */
public class DeploymentFormOfferMetadata extends EntityMetadata implements
		IsSerializable {

	public DeploymentFormOfferMetadata() {

		field(FieldMetadata.create(PaaSOfferingModel.TITLE).label("Offer name")
				.editType(TEXT).tooltip("Offer name").readonly(true));
		field(FieldMetadata.create(PaaSOfferingModel.PROVIDER)
				.label("Provider").editType(TEXT).tooltip("Offer provider")
				.readonly(true));
	}
}
