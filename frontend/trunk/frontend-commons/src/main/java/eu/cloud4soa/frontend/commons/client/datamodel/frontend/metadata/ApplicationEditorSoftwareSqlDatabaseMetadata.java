...
/**
 * SQL database form metadata
 *
 * @author Stefano Travelli (Cyntelix)
 */
public class ApplicationEditorSoftwareSqlDatabaseMetadata extends EntityMetadata implements IsSerializable {

    public ApplicationEditorSoftwareSqlDatabaseMetadata() {

        field(FieldMetadata.create(SOFTWARE_CATEGORY)
                .label("Category")
                .editType(COMBOBOX)
                .tooltip("SQL Database component category")
                .relatedEntityType(COMBO_SQL_DB_CATEGORY)
        );

        field(FieldMetadata.create(DB_CONFIGURATION, CAPACITY, MIN)
                .label("Database min capacity")
                .editType(EditType.EDIT_MEASURE_VALUE)
                .withRequiredFlag(DB_CONFIGURATION, CAPACITY, REQUIRED)
                .allowDecimals(false)
                .allowNegative(false)
                .tooltip("Type the required min database capacity, e.g. 10 Gb")
        );

        field(FieldMetadata.create(DB_CONFIGURATION, CACHE, MIN)
        ...
        field(FieldMetadata.create(REQUIRED)
                .label(Strings.EMPTY)
                .boxLabel("SQL Database is required.")
                .editType(CHECKBOX)
                .tooltip("Check if this SQL Database is required.")
        );
