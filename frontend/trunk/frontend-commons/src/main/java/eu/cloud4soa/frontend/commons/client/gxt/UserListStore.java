 ...
 */
public class UserListStore extends ListStore<UserModel> {
    ...
    public UserListStore() {
        ...
        super(new BasePagingLoader<PagingLoadResult<UserModel>>(new RpcProxy<PagingLoadResult<UserModel>>() {
            private ModelManagerServiceAsync modelManagerService = GWT.create(ModelManagerService.class);

            @Override
            protected void load(Object loadConfig, AsyncCallback<PagingLoadResult<UserModel>> pagingLoadResultAsyncCallback) {
                final PagingLoadConfig config = (PagingLoadConfig) loadConfig;
                modelManagerService.retrieveAllUsers(config, pagingLoadResultAsyncCallback);
            }
        }));
    }
}
