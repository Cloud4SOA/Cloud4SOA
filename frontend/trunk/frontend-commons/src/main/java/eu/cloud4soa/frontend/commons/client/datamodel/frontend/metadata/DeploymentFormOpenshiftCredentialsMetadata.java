...
/**
 * Credentials form for Openshift
 * 
 * @author Stefano Travelli (Cyntelix)
 */
public class DeploymentFormOpenshiftCredentialsMetadata extends EntityMetadata implements
		IsSerializable {
	public DeploymentFormOpenshiftCredentialsMetadata() {

		field(FieldMetadata.create(UserCredentialsModel.PUBLIC_KEY)
				.label("Email").editType(TEXT)
				.tooltip("Email used as the Openshift account")
				.allowBlank(false));
		field(FieldMetadata.create(UserCredentialsModel.PRIVATE_KEY)
				.label("Password").editType(TEXT)
				.tooltip("The Openshift password associated with the email")
